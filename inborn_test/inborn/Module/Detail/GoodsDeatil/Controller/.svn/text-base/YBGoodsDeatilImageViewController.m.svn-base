...
//    [SVProgressHUD show];
    NSString *GOODS_URL;
    if (self.isPreview) {
        GOODS_URL = COMPOSE_PREVIEW_URL;
    }else{
        GOODS_URL = GOODS_DEATIL_BIGIMAGE_URL;
    }
    [[ZJProjectNetRequestManager sharedNetRequestManager] connectNetWithUrl:GOODS_URL
                                                         requestNetworkType:ZJProjectNetRequestPost
                                                                 parameters:@{@"goodsId":_goodsId ? _goodsId : @""}
                                                            timeoutInterval:6
                                                               successBlock:^(id objc, id requestInfo) {
                                                                   NSString *status = [NSString stringWithFormat:@"%@",[objc objectForKey:@"status"]];
...
- (void)viewController:(PBViewController *)viewController presentImageView:(UIImageView *)imageView forPageAtIndex:(NSInteger)index progressHandler:(void (^)(NSInteger, NSInteger))progressHandler
{
    ...
    [imageView sd_setImageWithURL:[NSURL URLWithString: ZJIMAGEURLSTRING(ZJProjectImageURLStringMid,
                                                                         imageUrl)]
...
- (YBBlankpageView *)blankPageView {
    if (!_blankPageView) {
        ...
        _blankPageView.tipsText = @"抱歉，未找到该商品！";
        _blankPageView.logImageNamed = @"notfindGoods_blank_bg";
    }
