...
- (void)customCamera
{
    ...
    if (error) {
        YBLog(@"取得设备输入对象时出错，错误原因：%@",error.localizedDescription);
        return;
    }
...
- (IBAction)onEnsureAction:(UIButton *)sender {
    ...
    if (self.saveVideoUrl) {
        ...
        [assetsLibrary writeVideoAtPathToSavedPhotosAlbum:self.saveVideoUrl completionBlock:^(NSURL *assetURL, NSError *error) {
            YBLog(@"outputUrl:%@",weakself.saveVideoUrl);
            ...
            if (error) {
                YBLog(@"保存视频到相簿过程中发生错误，错误信息：%@",error.localizedDescription);
                [Utility showAllTextDialog:SYSTEM_KEYWINDOW Text:@"保存视频到相册发生错误"];
            } else {
...
- (void)videoHandlePhoto:(NSURL *)url {
    ...
    if (error) {
        YBLog(@"截取视频图片失败:%@",error.localizedDescription);
    }
