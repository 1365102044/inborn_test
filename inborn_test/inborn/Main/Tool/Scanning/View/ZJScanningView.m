 ...
 *  扫描内容的Y值
 */
#define scanContent_Y self.frame.size.height * 0.20
...
@property(nonatomic,strong) UIButton * light_desc_button;
@end
 ...
 *  定时器和动画的时间
 */
static CGFloat const timer_animation_Duration       = 0.1;
...
- (void)setupScanningCodeEdging
{
    ...
    promptLabel.font = SYSTEM_REGULARFONT(13);
    promptLabel.textColor = ZJCOLOR.color_c0;
     ...
     *  添加闪光灯按钮
     */
    UIButton *light_desc_button = [[UIButton alloc] init];
    CGFloat light_desc_buttonX = 0;
    CGFloat light_desc_buttonH = 25;
    CGFloat light_desc_buttonY = self.height - 64 - ADJUST_PERCENT_FLOAT(40) - light_desc_buttonH;
    CGFloat light_desc_buttonW = self.frame.size.width;
    light_desc_button.frame = CGRectMake(light_desc_buttonX, light_desc_buttonY, light_desc_buttonW, light_desc_buttonH);
    [light_desc_button setTitle:@"打开照明灯" forState:UIControlStateNormal];
    [light_desc_button setTitleColor:ZJCOLOR.color_c0 forState:(UIControlStateNormal)];
    light_desc_button.titleLabel.font = SYSTEM_REGULARFONT(14);
    [self addSubview:light_desc_button];
    self.light_desc_button = light_desc_button;
    
    
    UIButton *light_button = [[UIButton alloc] init];
    CGFloat light_buttonH = ADJUST_PERCENT_FLOAT(50);
    CGFloat light_buttonX = (self.frame.size.width - light_buttonH)/2;
    CGFloat light_buttonW = light_buttonH;
    CGFloat light_buttonY = light_desc_button.frame.origin.y - light_buttonH - 10;
    light_button.frame = CGRectMake(light_buttonX, light_buttonY, light_buttonW, light_buttonH);
    [light_button setImage:[UIImage imageNamed:@"dimension_electrictorch_n"] forState:UIControlStateNormal];
    [light_button setImage:[UIImage imageNamed:@"dimension_electrictorch_h"] forState:UIControlStateSelected];
...
- (void)light_buttonAction:(UIButton *)button {
    if (button.selected == NO) {
        ...
        button.selected = YES;
        [self.light_desc_button setTitle:@"关闭照明灯" forState:UIControlStateNormal];
    } else {
        [self turnOnLight:NO];
        button.selected = NO;
        [self.light_desc_button setTitle:@"打开照明灯" forState:UIControlStateNormal];
    }
}
