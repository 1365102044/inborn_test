...
static float const kPopoverViewCellHeight = 32.f; ///< cell指定高度
static float const kPopoverViewArrowHeight = 3.15f; ///< 箭头高度
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    // 截取弹窗时相关数据
    float arrowWidth = ADJUST_PERCENT_FLOAT(8.f);
    float cornerRadius = 4.f;
    float arrowCornerRadius = 0.f;
    float arrowBottomCornerRadius = 0.f;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return ADJUST_PERCENT_FLOAT(kPopoverViewCellHeight);
}
