...
#import "YBTabInputContentView.h"
#import "YBVoiceTipsView.h"

static CGFloat const kMQChatViewInputBarHeight = 50.f;

@interface YBChatViewController () <UITableViewDelegate, MQChatViewServiceDelegate, MQInputToolBarDelegate, UIImagePickerControllerDelegate, MQChatTableViewDelegate, MQChatCellDelegate, MQServiceToViewInterfaceErrorDelegate,UINavigationControllerDelegate, MQEvaluationViewDelegate, MQInputContentViewDelegate, MQKeyboardControllerDelegate, MQRecordViewDelegate, MQRecorderViewDelegate, MQAGEmojiKeyboardViewDelegate, MQAGEmojiKeyboardViewDataSource, YBRecorderViewDelegate>
...
@property (nonatomic, strong) YBInputToolBar *chatInputBar;
...
/**
 其他功能视图
 */
@property (nonatomic, strong) ZJBaseView *otherFuncView;
...
- (void)voiceValueChange:(NSNotification *)no
{
    NSDictionary *userInfo = no.userInfo;
    YBLog(@"%@", userInfo);
    [self.voiceTipsView setVoiceValueWithNumber:[userInfo[@"value"] integerValue]];
}
...
{
    YBLog(@"%zd", type);
    if (type == YBRecorderViewBegan) {
        [self.voiceTipsView setTipsImageWithIsCancel:YES];
        self.voiceTipsView.alpha = 1.0f;
    }else if (type == YBRecorderViewChangeCancel) {
        [self.voiceTipsView setTipsImageWithIsCancel:NO];
    }else if (type == YBRecorderViewChangeContinue) {
        [self.voiceTipsView setTipsImageWithIsCancel:YES];
    }else if (type == YBRecorderViewEnd) {
        self.voiceTipsView.alpha = 0.0f;
    }
...
- (void)viewDidLoad {
    ...
    self.chatInputBar.contentViewDelegate = self;
}
        ...
        do {
            if (self.chatViewService.clientStatus == MQStateAllocatingAgent || [NSDate timeIntervalSinceReferenceDate] - sendTime < 1) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (self.urlString) {
                        YBLog(@"%@\n%@", self.urlString, [NSThread currentThread]);
                        [self sendTextMessage:self.urlString];
                        self.urlString = nil;
                    }
                });
            }
            
        } while (self.urlString);
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [SYSTEM_NOTIFICATIONCENTER removeObserver:self name:@"MQVoiceValue" object:nil];
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [SYSTEM_NOTIFICATIONCENTER addObserver:self selector:@selector(voiceValueChange:) name:@"MQVoiceValue" object:nil];
}
...
{
    if (!_chatViewService) {
        _chatViewService = [[MQChatViewService alloc] initWithDelegate:self errorDelegate:self];
    }
    return _chatViewService;
}
...
- (void)initChatTableView {
    ...
    self.chatTableView.backgroundColor = ZJCOLOR.color_c12;
...
- (void)otherFunc {
    if ([self handleSendMessageAbility]) {
        if (self.chatInputBar.isFirstResponder) {
            ...
            ZJBaseView *baseView = [[ZJBaseView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, ADJUST_PERCENT_FLOAT(224.f))];
            
            NSArray *tempArray = @[@{@"imageNamed":@"message_photos",
                                     @"title":ZJSTRING(@"相册")},
                                   @{@"imageNamed":@"message_camera",
                                     @"title":ZJSTRING(@"拍摄")}];
            
            [tempArray enumerateObjectsUsingBlock:^(NSDictionary *  _Nonnull obj,
                                                    NSUInteger idx,
                                                    BOOL * _Nonnull stop) {
                YBVerticalButton *verticalButton = [[YBVerticalButton alloc] init];
                
                NSArray *tempImageArray = ZJIMAGE(IMAGEFILEPATH_MESSAGE, obj[@"imageNamed"], ZJProjectLoadImageDefault);
                [verticalButton setImage:tempImageArray.firstObject forState:UIControlStateNormal];
                [verticalButton setImage:tempImageArray.lastObject forState:UIControlStateHighlighted];
                [verticalButton setTitleColor:ZJCOLOR.color_c4 forState:UIControlStateNormal];
                [verticalButton addTarget:self action:@selector(funcButtonClick:) forControlEvents:UIControlEventTouchUpInside];
                [verticalButton setTitle:obj[@"title"] forState:UIControlStateNormal];
                
                verticalButton.titleLabel.font = SYSTEM_REGULARFONT(12.f);
                verticalButton.tag = 1001 + idx;
                [baseView addSubview:verticalButton];
                
                [verticalButton mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.height.mas_equalTo(ADJUST_PERCENT_FLOAT(84.f));
                    make.width.mas_equalTo(ADJUST_PERCENT_FLOAT(60.f));
                }];
                
                if (idx == 0) {
                    [verticalButton mas_makeConstraints:^(MASConstraintMaker *make) {
                        make.left.mas_equalTo(baseView).mas_offset(ADJUST_PERCENT_FLOAT(20.f));
                        make.top.mas_equalTo(baseView).mas_offset(ADJUST_PERCENT_FLOAT(16.f));
                    }];
                }else if (idx <= 3) {
                    if (idx == 3) {
                        [verticalButton mas_makeConstraints:^(MASConstraintMaker *make) {
                            make.left.mas_equalTo(baseView.subviews[idx - 1].mas_right).mas_offset(ADJUST_PERCENT_FLOAT(31.65));
                            make.right.mas_equalTo(baseView).mas_offset(ADJUST_PERCENT_FLOAT(-20.f));
                            make.top.mas_equalTo(baseView.subviews[idx - 1]);
                        }];
                    }else{
                        [verticalButton mas_makeConstraints:^(MASConstraintMaker *make) {
                            make.left.mas_equalTo(baseView.subviews[idx - 1].mas_right).mas_offset(ADJUST_PERCENT_FLOAT(31.65));
                    ...
                    if (idx == 4) {
                        [verticalButton mas_makeConstraints:^(MASConstraintMaker *make) {
                            make.left.mas_equalTo(baseView.subviews.firstObject);
                            make.top.mas_equalTo(baseView.subviews.firstObject.mas_bottom).mas_offset(ADJUST_PERCENT_FLOAT(15.f));
                        }];
                    }else{
                        [verticalButton mas_makeConstraints:^(MASConstraintMaker *make) {
                            make.left.mas_equalTo(baseView.subviews[idx - 1].mas_right).mas_offset(ADJUST_PERCENT_FLOAT(31.65));
            ...
            self.chatInputBar.inputView = baseView;
            [self.chatInputBar becomeFirstResponder];
...
- (void)inputBar:(YBInputToolBar *)inputBar willChangeHeight:(CGFloat)height {
    if (height > kMQChatViewInputBarHeight) {
        ...
        [self changeInputBarHeightConstraintConstant:height + 10];
    }else{
...
- (YBInputToolBar *)chatInputBar {
    if (!_chatInputBar) {
        ...
        YBTabInputContentView *contentView = [[YBTabInputContentView alloc] initWithFrame:CGRectMake(0, 44, self.view.frame.size.width, 44)];
        _chatInputBar = [[YBInputToolBar alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 44) contentView: contentView];
...
- (YBRecorderView *)displayRecordView {
    if (!_displayRecordView) {
        _displayRecordView = [[YBRecorderView alloc] initWithFrame: CGRectMake(0, 0, self.view.frame.size.width, 258)];
...
- (YBVoiceTipsView *)voiceTipsView
{
    if (!_voiceTipsView) {
        ...
        _voiceTipsView.alpha = 0.f;
        [self.view addSubview:_voiceTipsView];
        [_voiceTipsView mas_makeConstraints:^(MASConstraintMaker *make) {
